...
INLINE double linToExpWithOffset(double in,
                                 double inMin,
                                 double inMax,
                                 double outMin,
                                 double outMax,
...
INLINE double expToLinWithOffset(double in,
                                 double inMin,
                                 double inMax,
                                 double outMin,
                                 double outMax,
...
INLINE double
amp2dBWithCheck(double amp, double lowAmplitude)
{
  if (amp >= lowAmplitude)
    return 8.6858896380650365530225783783321 * log(amp);
  else
...
template <class T>
INLINE int
arrayMaxIndex(T *theArray, int numValues)
{
  int maxIndex = 0;
...
template <class T>
INLINE int
arrayMinIndex(T *theArray, int numValues)
{
  int minIndex = 0;
...
INLINE double
beatsToSeconds(double beat, double bpm)
{
...
INLINE double
degreeToRadiant(double degrees)
{
...
INLINE double
freqToPitch(double freq)
{
  return 12.0 * rosic_log2(freq / 440.0) + 69.0;
}

INLINE double
freqToPitch(double freq, double masterTuneA4)
{
...
INLINE float
indexToNormalizedValue(int index, int numIndices)
{
  return (float)(2 * index + 1) / (float)(2 * numIndices);
}

INLINE bool
isCloseTo(double x, double targetValue, double tolerance)
{
...
INLINE double
logB(double x, double b)
{
  return log(x) / log(b);
}

INLINE double
linToLin(double in, double inMin, double inMax, double outMin, double outMax)
{
...
INLINE double
linToExp(double in, double inMin, double inMax, double outMin, double outMax)
{
  // map input to the range 0.0...1.0:
  double tmp = (in - inMin) / (inMax - inMin);
...
INLINE double
expToLin(double in, double inMin, double inMax, double outMin, double outMax)
{
...
INLINE int
normalizedValueToIndex(float normalizedValue, int numIndices)
{
  return (int)floor(normalizedValue * numIndices);
}

INLINE double
pitchOffsetToFreqFactor(double pitchOffset)
{
...
INLINE double
pitchToFreq(double pitch, double masterTuneA4)
{
...
INLINE double
radiantToDegree(double radiant)
{
...
#ifdef _WIN32
INLINE double
randomUniform(double min, double max, int seed)
{
...
INLINE double
secondsToBeats(double timeInSeconds, double bpm)
{
...
INLINE double
wholeNotesToSeconds(double noteValue, double bpm)
{
